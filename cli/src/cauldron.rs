...
impl App {
    ...
    fn list_next(&mut self) {
        let (state, len) = match self.focused_pane {
            ...
            FocusedPane::Content => return, // No list nav in content pane... yet
        };
        ...
        if event::poll(timeout)? {
            if let Event::Key(key) = event::read()? {
                match key.code {
                    ...
                    // --- NEW: Focus and navigation ---
                    KeyCode::Tab => app.cycle_focus(),
                    KeyCode::Down | KeyCode::Char('j') => app.list_next(),
                    KeyCode::Up | KeyCode::Char('k') => app.list_previous(),

                    // TODO: Add 'h'/'l' for content tab switching
                    KeyCode::Char('l') => {
                        if app.focused_pane == FocusedPane::Content {
                            // Cycle right-hand tabs
                            app.content_tab_index = (app.content_tab_index + 1) % 2; // 2 tabs for now
                        }
                    }
                    KeyCode::Char('h') => {
                         if app.focused_pane == FocusedPane::Content {
...
fn ui<B: Backend>(f: &mut Frame<B>, app: &mut App) {
    ...
    let p = Paragraph::new(format!("\nDetails for selected item...\n\nContent Tab Index: {}\nFocused Pane: {:?}{}", 
            app.content_tab_index,
            match app.focused_pane {
                FocusedPane::Containers => "Containers",
                FocusedPane::Manifests => "Manifests",
                FocusedPane::Homes => "Homes",
                FocusedPane::Content => "Content",
            },
            help_text
        ))
        .block(Block::default().borders(Borders::TOP).border_style(default_style)); // Line separator
    f.render_widget(p, main_content_area);
}