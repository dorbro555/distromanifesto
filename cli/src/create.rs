...
impl<'a> App<'a> {
    fn new(file_path: &Path, section_name: String) -> App<'a> {
        let mut state = ListState::default();
        state.select(Some(0)); // Select the first section

        let items = vec![DisplayItem::Section(section_name.clone())];

        let mut app = App {
            file_path: file_path.to_path_buf(),
            items,                  // <-- Use manual items
            list_items: Vec::new(), // Will be populated by refresh
            state,
            mode: AppMode::Navigating,
            value_input: Input::default(),
            current_section: section_name, // <-- Set current section
...
fn ui<B: Backend>(f: &mut Frame<B>, app: &mut App) {
    ...
    let (footer_text, footer_style) = match &app.mode {
        AppMode::Navigating => {
            ...
            // Set style to Yellow if not valid
            ...
            // Add the warning message to the title text
